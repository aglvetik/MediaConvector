...
async def test_music_audio_cache_reuse(service_harness) -> None:
    ...
    assert service_harness.jamendo_provider.download_calls["after_dark"] == 1
...
async def test_music_invalid_cached_file_id_rebuild(service_harness) -> None:
    ...
    assert service_harness.jamendo_provider.download_calls["after_dark"] == 2
...
async def test_music_strategy_fallback_uses_internet_archive_when_jamendo_download_fails(service_harness) -> None:
    query = "\u043d\u0430\u0439\u0442\u0438 fallback anthem"
    service_harness.jamendo_provider.fail_download_ids.add("fallback_anthem")

    await service_harness.process_message_service.handle_message(
        IncomingMessage(chat_id=1, user_id=505, message_id=7, chat_type="private", text=query)
    )

    assert service_harness.jamendo_provider.download_attempts == ["fallback_anthem"]
    assert service_harness.internet_archive_provider.search_calls["fallback anthem"] == 1
    assert service_harness.internet_archive_provider.download_attempts == ["fallback_anthem"]
    ...
    assert cache_entry is not None
    assert cache_entry.acquisition_backend == "internet_archive"
...
async def test_music_pipeline_tries_next_candidate_when_first_candidate_fails(service_harness) -> None:
    ...
    service_harness.jamendo_provider.results["complex fallback"] = [
        MusicTrack(
            source_id="first_candidate",
            source_url="https://catalog.example/jamendo/first_candidate",
            canonical_url="https://catalog.example/jamendo/first_candidate",
        ...
        MusicTrack(
            ...
            source_url="https://catalog.example/jamendo/second_candidate",
            canonical_url="https://catalog.example/jamendo/second_candidate",
    ...
    service_harness.jamendo_provider.fail_download_ids.add("first_candidate")
    ...
    assert service_harness.jamendo_provider.download_attempts == ["first_candidate", "second_candidate"]
...
async def test_music_cache_persists_primary_acquisition_backend(service_harness) -> None:
    ...
    cache_entry = await service_harness.cache_service.get_entry("music:ytm:after dark")
    assert cache_entry is not None
    assert cache_entry.acquisition_backend == "jamendo"
