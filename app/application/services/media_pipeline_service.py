...
class MediaPipelineService:
    ...
    async def process(self, request: MediaRequest, provider: DownloaderProvider) -> MediaResult:
        cache_entry = await self._cache_service.get_reusable(request.normalized_resource.normalized_key)
        ...
        if joined:
            shared_cache = await self._cache_service.get_reusable(request.normalized_resource.normalized_key)
    ...
    async def _refresh_missing_audio(
        ...
        if joined:
            refreshed = await self._cache_service.get_reusable(request.normalized_resource.normalized_key)
