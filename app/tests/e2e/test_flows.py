...
async def test_tiktok_music_link_is_rejected_with_clear_user_message(service_harness) -> None:
    handled = await service_harness.process_message_service.handle_message(
        IncomingMessage(
            chat_id=1,
            user_id=303,
            message_id=21,
            chat_type="private",
            text="https://www.tiktok.com/music/original-sound-777777",
        )
    )
    ...
    assert len(service_harness.gateway.sent_audio_receipts) == 0
    assert service_harness.gateway.text_messages[-1].text == messages.TIKTOK_MUSIC_LINKS_NOT_SUPPORTED
...
async def test_supported_tiktok_url_later_in_message_is_processed(service_harness) -> None:
    handled = await service_harness.process_message_service.handle_message(
        IncomingMessage(
            chat_id=1,
            user_id=408,
            message_id=30,
            chat_type="private",
            text="bad https://example.com/thing and then https://www.tiktok.com/@user/video/424242",
        )
    )
    assert handled is True
    assert service_harness.provider.download_calls["tiktok:video:424242"] == 1
