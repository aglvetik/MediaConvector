...
class FakeProvider:
    ...
    async def normalize(self, url: str) -> NormalizedResource:
        ...
        if "/photo/" in url:
            ...
            image_urls = tuple(
                f"https://example.com/{resource_id}-{index}.jpg"
                for index in range(1, self.photo_counts[normalized_key] + 1)
            )
        ...
        return NormalizedResource(
            ...
            media_kind="gallery"
            if resource_type == "photo_post" and len(image_urls) > 1
            else ("photo" if resource_type == "photo_post" else "video"),
            title=resource_type,
            author="author",
            audio_url=audio_url,
            image_urls=() if normalized_key in self.download_first_gallery_keys else image_urls,
            image_entries=()
            if normalized_key in self.download_first_gallery_keys
            else tuple(
            ...
            has_expected_audio=(
                None
                if normalized_key in self.download_first_gallery_keys and resource_type == "photo_post"
                else (audio_url is not None if resource_type != "video" else None)
            ),
        )
    ...
    async def download_images(self, normalized: NormalizedResource, work_dir: Path) -> tuple[Path, ...]:
        ...
        for index in range(1, total + 1):
            path = await self.download_image_entry(
                ...
                work_dir,
                source_url=(
                    normalized.image_urls[index - 1]
                    if normalized.image_urls
                    else f"https://example.com/{normalized.resource_id}-{index}.jpg"
                ),
                entry_index=index,
            )
            paths.append(path)
...
class FakeFfmpegAdapter:
    ...
    async def transcode_audio_to_mp3(
        ...
        *,
        ...
        del source_path, title, performer, cover_path
...
class FakeGateway:
    ...
    async def send_video_by_file_id(
        ...
        del chat_id, caption, reply_to_message_id
    ...
    async def send_photo_group_by_upload(
        ...
        receipts = [await self.send_photo_by_upload(0, path) for path in file_paths]
    ...
    async def send_photo_group_by_file_id(
        ...
        receipts = [await self.send_photo_by_file_id(0, file_id) for file_id in file_ids]
        return tuple(receipts)
